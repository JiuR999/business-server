...
func main() {
	...
	mysqlDsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8mb4&parseTime=True&loc=Local", userName, password, address, port, database)
	...
	g.GenerateModel("swust_system_role", fieldOpts...)
	g.Execute()
}
